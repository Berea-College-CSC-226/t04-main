...
```
    Change the variable name so there were no capitals, and we also had to correct the grammar for a comment changing
    “Idk” to “I don’t know” and a adding a comma before a conjunction 
```
...
```        
    The files that we created and or the changes do not appear in main branch.
```
